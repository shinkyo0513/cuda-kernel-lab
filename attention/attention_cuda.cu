...
void launch_attention_cuda_fused_softmax(
    const float *d_Q,
    const float *d_K,
    const float *d_V,
    float *d_O,
...
void launch_attention_cuda_fused_softmax_with_workspace(
    const float *d_Q,
    const float *d_K,
    const float *d_V,
    float *d_O,
    ...
    int dim)
{
    ...
    // 4. probs = fused softmax(scores)
    launch_softmax_fused(d_scores, d_probs, S, S);
    ...
    int dim)
{
    ...
    // K_T:    [D, S]
    // scores: [S, S]
    launch_matmul_cublas(handle, d_Q, d_KT, d_scores, S, S, D);
    ...
    // V:     [S, D]
    // O:     [S, D]
    launch_matmul_cublas(handle, d_probs, d_V, d_O, S, D, S);
}
